...
pub fn rules() -> Html {
    html! {
        <section id="rules">
            ...
            <p>{"Si le pion finit son mouvement sur une case occupée, il peut alors rebondir  et recommencer un mouvement d'autant de cases que la valeur du pion sur lequel il est arrivé. Un même pion peut bien entendu rebondir plusieurs fois de suite. Il est également possible de remplacer le pion d'arrivé par le pion qui s'est déplacé. Vous devrez alors repositionner le pion d'arrivée sur une case vide de votre choix n'importe où sur le plateau."}</p>
